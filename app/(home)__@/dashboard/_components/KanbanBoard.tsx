...
import { T_TaskWithAllData } from '@/actions/tasks/getAllTasksWhereUserIsMemeber';
...
const KanbanBoard = ({ tasks }: { tasks: T_TaskWithAllData[] | null }) => {
  const [allTasks, setAllTasks] = useState<T_TaskWithAllData[]>(tasks || []);
  const [activeTask, setActiveTask] = useState<T_TaskWithAllData | null>(null);
  ...
  let notStartedTasks: T_TaskWithAllData[] | null = allTasks.filter(
    (task) => task.taskStatus === 'NOT_STARTED'
  );
  notStartedTasks = sortTasks(notStartedTasks, 'date', 'down');

  let inProgressTasks: T_TaskWithAllData[] | null = allTasks.filter(
    (task) => task.taskStatus === 'IN_PROGRESS'
  );

  inProgressTasks = sortTasks(inProgressTasks, 'date', 'down');

  let completedTasks: T_TaskWithAllData[] | null = allTasks.filter(
