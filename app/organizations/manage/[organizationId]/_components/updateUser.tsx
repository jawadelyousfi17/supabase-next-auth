...
interface T_Props {
  member: T_Member;
  addOptimisticAtion: (action: OptimisticAction) => void;
}

export const UpdateMemberPreference = ({
  member,
  addOptimisticAtion,
}: T_Props) => {
  ...
  const handleSave = async () => {
    setLoading(true);
    const { error, message } = await updateMemeberPref(
      member?.id as string,
      organizationId,
      role
    );
  ...
  const handleDelete = async () => {
    setLoading2(true);
    const { error, message } = await deleteMemeber({
