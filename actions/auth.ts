...
async function signInWithEmail(prevState: any, formData: FormData) {
    ...
    const { data, error } = await supabase.auth.signInWithOtp({
        ...
        options: {
            ...
            emailRedirectTo:
                'http://localhost:3000/auth/otp-magic-link/callback',
        },
    });
...
const resetPassword = async (prevState: any, formData: FormData) => {
    ...
    if (error)
        return { error: "can't send reset link for now, try again later" };

    return { message: 'confiramtion link sent! check your email' };
};

const updatePasswordPasswordless = async (
    code: string,
    prevState: any,
    formData: FormData
) => {
    ...
    if (error) return { error: 'Invalid or expired link' };

    {
        ...
        await supabase.auth.signOut();
        if (error) return { error: "can't update the password" };
